...
df2021 <- edvr.workshop::census_bpsex %>% 
  filter(census == 2021) %>% 
  filter(!birth %in% c("Total")) %>% 
  select(-census) %>% 
  summarise(count = sum(count), .by = birth) %>% 
...
## India now third most common place of birth of Australian residents, census results show {background-color="#746FB2"}
...
total2021 <- sum(df2021$count)
auperc2021 <- df2021 %>% 
  filter(birth=="Australia") %>% 
  pull(percentage)
nsperc2021 <- df2021 %>% 
  filter(birth=="Not Stated") %>% 
  pull(percentage)
df2021 %>% 
